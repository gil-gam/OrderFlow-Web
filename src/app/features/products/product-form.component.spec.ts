...
describe('ProductFormComponent', () => {
  ...
  const mockCategories = [
    { id: '1', name: 'Electronics', description: 'Eletrônicos', isActive: true, createdAt: baseDate },
    { id: '2', name: 'Clothing', description: 'Vestuário', isActive: true, createdAt: baseDate },
  ];
  const mockProduct = {
    id: '1', name: 'Laptop', description: 'Good', unitPrice: 1500, currency: 'USD',
    stockQuantity: 10, categoryId: '1', categoryName: 'Electronics',
    isActive: true, createdAt: baseDate,
  };
  ...
  it('should call create on submit', async () => {
    ...
    expect(productServiceMock.create).toHaveBeenCalledWith({
      name: 'Mouse', description: '', categoryId: '1', unitPrice: 50, currency: 'USD',
    });
  });

  it('should navigate to /products on success', async () => {
    await createComponent();
    productServiceMock.create.and.returnValue(of({ id: '3' } as any));
    component.form.patchValue({ name: 'Mouse', price: 50, stockQuantity: 100, categoryId: '1' });
    component.onSubmit();
